...
> *"Souhaitez-vous également supprimer les applications ET leurs données gérées par Docker Gate ? Réponds explicitement 'oui' ou 'non' :"* (this prompt is only shown in French for now, matching the rest of this script's messages — see note below)

An **explicit** answer is required — a bare Enter or any other input loops back to the question ("Réponse non reconnue" / "answer not recognized") instead of silently choosing for you. Answering **oui** (yes) actually removes each app (Docker container, data volume, YunoHost exposure, dedicated domain if any) before continuing to uninstall Docker Gate itself — a genuine full cleanup in one go. Answering **non** (no) leaves the apps untouched, exactly as described above (findable via `/audit` after a later reinstall). This question is only shown when a real terminal is attached (never during an automated/API removal — in that case, the safest default applies: nothing is touched).
