...
> 💡 **For the easiest install, prefer the URL option** (the raw link to the project's `docker-compose.yml`, e.g. on GitHub) over pasting the text. When fetched by URL, Docker Gate also looks for the project's own example environment file next to it (`.env.example`, `example.env`...) and pre-fills the required variables automatically from it — no need to look them up in the app's docs and type them by hand. This isn't a Docker Gate-maintained list of apps: it only ever reads what the upstream project already publishes next to its own compose file.

If the compose declares **more than one service** (e.g. an app + its own database + a cache — common for apps like Immich, Paperless-ngx...), Docker Gate lets you pick which one is exposed via YunoHost/SSO; the others start alongside it as internal dependencies on a dedicated Docker network, never publicly reachable, never with a published port. Passwords found on those internal-only services are generated automatically (strong random values, consistently propagated wherever the same value is referenced) instead of being left for you to type and keep in sync by hand.
...
**Second question asked at uninstall time (since 2026-07-18)**: right after the one above, if Docker CE is installed on the machine, a second question offers to fully uninstall it too (`docker-ce`/`containerd.io`/etc. packages, `/var/lib/docker`, the system `docker` group) — symmetric to the fact that Docker Gate installs it automatically on first install. **Before asking, any container still present is reported**, split into two categories: apps still tracked and kept by Docker Gate (e.g. you answered "no" to the first question) and containers unrelated to Docker Gate entirely — purging Docker CE destroys both equally, so both are called out explicitly (an earlier version of this warning only covered the second category, wrongly implying it was safe to proceed whenever no unrelated container existed). Same mechanics as the first question: an explicit answer is required, never blocking outside a real terminal (Docker CE is then left installed by default, the safest behavior).

**⚠️ Recommended order if you intend to remove Docker Gate, its child apps, AND Docker CE all at once**: do it from the command line, where both interactive questions above can actually be answered. **If you remove Docker Gate from the YunoHost admin panel (GUI) instead**, neither question can ever appear (no terminal is attached to a web-triggered action) — Docker Gate silently defaults to the safest choice each time (child apps kept, Docker CE left installed), and the job log only states this after the fact. If you plan to also uninstall Docker CE, **do it first, from Docker Gate's own Audit page (the "Uninstall Docker CE" button)**, before removing Docker Gate itself — once Docker Gate is uninstalled, that button (and its warning about which apps depend on Docker CE) is gone along with it, and you'd otherwise have to purge Docker CE by hand with no built-in warning about what it will break.
...
- **Execution engine (rewritten 2026-07-20)**: every app — one container or several — is run through a real `docker compose` invocation, driven from a minimal `docker-compose.yml` Docker Gate generates itself under `data/compose/<slug>/`, built only from the already-parsed fields (image/port/volume/env), never from the user's raw pasted text. Keys with real risk on a shared host (`build`, `privileged`, `cap_add`, `devices`, `network_mode: host`, `secrets`, `extends`) are never read by the parser in the first place, so they can never reach the generated file either. Each multi-container app gets its own dedicated Docker network; containers reach each other by their original compose service name (Compose's native DNS resolution — no custom code needed for that).
- **State**: each created app is recorded in `data/apps.json` (slug, image, ports, domain/path, optional volume, and — for multi-container apps — its companions and dedicated network). This is Docker Gate's only source of truth about what it manages — a Docker container/volume/network that exists but doesn't appear in this file is treated as a residue (see the Audit page).
...
- **Removal (of a child app)**: done in 3 separately-verified layers (SSO permission + nginx config, Docker container(s) via `docker compose down`, state entry) — each step is attempted independently; a failure on one step never blocks the next ones, and the entry is always removed from the state at the end (a real residue stays detectable via the Audit page rather than blocking indefinitely). Apps created before the 2026-07-20 rewrite are removed through the older direct-Docker-API path instead, kept as a compatibility fallback — no forced migration.
